...
class FicheConsultationViewSet(viewsets.ModelViewSet):
    ...
    # ------- Actions -------
    @extend_schema(
        tags=["Consultations"], summary="Valider la consultation", responses={200: FicheConsultationSerializer}
    )
    ...
    @extend_schema(
        ...
        responses={202: OpenApiResponse(description="Analyse relancée")},
    )
    @action(
        detail=True,
        methods=["post"],
    ...
    def edit_diagnostic(self, request, pk=None):
        ...
        if updated:
            ...
            return Response(
                {"detail": "Diagnostic médecin mis à jour avec succès", "fiche": self.get_serializer(fiche).data}
            )
        else:
    ...
    @action(
        detail=True, methods=["get", "post"], permission_classes=[permissions.IsAuthenticated], url_path="references"
    )
    def fiche_references(self, request, pk=None):
        ...
        allowed = (
            (fiche.user_id == u.id)
            or (fiche.assigned_medecin_id == u.id)
            or u.is_staff
            or getattr(u, "role", None) == "medecin"
        )
        if not allowed:
        ...
        # Éviter les doublons
        existing = FicheReference.objects.filter(fiche=fiche, title=serializer.validated_data["title"]).exists()

        if existing:
            return Response(
                {"detail": "Cette référence existe déjà pour cette fiche"}, status=status.HTTP_400_BAD_REQUEST
            )

        reference = FicheReference.objects.create(fiche=fiche, **serializer.validated_data)

        return Response(FicheReferenceSerializer(reference).data, status=status.HTTP_201_CREATED)
    ...
    def send_notification(self, request, pk=None):
        ...
        try:
            ...
            if result.success:
                ...
                return Response(
                    {"detail": f"Erreur envoi {method}: {result.error}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    ...
    def send_whatsapp(self, request, pk=None):
        ...
        try:
            ...
            if result.success:
                ...
                if (
                    settings.DEBUG
                    or "non configuré" in str(result.error)
                    or "not configured" in str(result.error).lower()
                ):
                    return Response(
                        {
                    ...
                    return Response(
                        {"detail": f"Erreur envoi WhatsApp: {result.error}", "fiche": fiche.id},
    ...
    def fiche_messages(self, request, pk=None):
        ...
        msg = FicheMessage.objects.create(fiche=fiche, author=u, content=content)
    ...
    def export_json(self, request, pk=None):
        ...
        fiche_data["references"] = FicheReferenceSerializer(fiche.references.all(), many=True).data
        fiche_data["messages"] = FicheMessageSerializer(fiche.messages.all(), many=True).data

        response = HttpResponse(json.dumps(fiche_data, indent=2, ensure_ascii=False), content_type="application/json")
    ...
    def field_choices(self, request):
        ...
        choices = {
            "coloration_bulbaire": [
                {"value": choice[0], "label": choice[1]} for choice in FicheConsultation.ColorationBulbaire.choices
            ],
            "coloration_palpebrale": [
                {"value": choice[0], "label": choice[1]} for choice in FicheConsultation.ColorationPalpebrale.choices
            ],
            "etat_civil": [{"value": choice[0], "label": choice[1]} for choice in FicheConsultation.EtatCivil.choices],
            "frequence": [{"value": choice[0], "label": choice[1]} for choice in FicheConsultation.Frequence.choices],
            "sexe": [{"value": "M", "label": "Masculin"}, {"value": "F", "label": "Féminin"}],
            "etat": [{"value": choice[0], "label": choice[1]} for choice in FicheConsultation.Etat.choices],
            "capacite": [{"value": choice[0], "label": choice[1]} for choice in FicheConsultation.Capacite.choices],
            "oui_non": [{"value": choice[0], "label": choice[1]} for choice in FicheConsultation.OuiNon.choices],
            "oui_non_inconnu": [
                {"value": choice[0], "label": choice[1]} for choice in FicheConsultation.OuiNonInconnu.choices
            ],
        }
...
Merci de votre confiance 🙏"""
        ...
        try:
            ...
            if result.success:
                ...
                return Response(
                    {"detail": f"Erreur envoi instructions: {result.error}", "phone": phone_number},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )

        except Exception as e:
            return Response(
                {"detail": f"Erreur service: {str(e)}", "phone": phone_number},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        tags=["Conversations"], summary="Récupérer une conversation (avec messages si serializer détail)"
    ),
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def confirm(self, request, pk=None):
        ...
        if not (start and end):
            return Response(
                {"detail": "confirmed_start et confirmed_end requis (ISO 8601)"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
...
class FicheAttachmentViewSet(viewsets.ModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"detail": f"Erreur de téléchargement: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class MedecinAvailabilityViewSet(viewsets.ModelViewSet):
    ...
    def calendar_ics(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": f"Erreur génération ICS: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def available_slots(self, request):
        ...
        if not date_start or not date_end:
            return Response({"detail": "Paramètres date_start et date_end requis"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response(
                {"detail": "Format de date invalide (YYYY-MM-DD attendu)"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Limiter à 4 semaines max
        if (end_date - start_date).days > 28:
            return Response({"detail": "Période maximale: 4 semaines"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if consultation_type:
            availabilities = availabilities.filter(consultation_type__in=[consultation_type, "both"])
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            if user:
                ...
                recent_fiche = FicheConsultation.objects.filter(user=user).order_by("-created_at").first()

                if recent_fiche:
                    webhook.related_fiche = recent_fiche

                    # Créer un message dans la fiche
                    message = FicheMessage.objects.create(fiche=recent_fiche, author=user, content=webhook.content)
...
class DataExportJobViewSet(
    ...
    def download(self, request, pk=None):
        ...
        if export_job.status != DataExportJob.ExportStatus.COMPLETED:
            return Response({"detail": "Export pas encore terminé"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response(
                {"detail": f"Erreur téléchargement: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
