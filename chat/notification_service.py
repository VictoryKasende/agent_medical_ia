...
class TwilioNotificationService:
    ...
    def send_sms(self, to_number: str, message: str, force_resend: bool = False) -> NotificationResult:
        """
        ...
        if not self.is_configured():
            return NotificationResult(success=False, error="Service Twilio non configuré")

        if not self.phone_number:
            return NotificationResult(success=False, error="Numéro Twilio SMS non configuré")
        ...
        if not force_resend and self._is_already_sent(cache_key):
            ...
            return NotificationResult(success=True, message_sid=cache.get(cache_key), status="already_sent")
        ...
        try:
            ...
            # Envoi via Twilio
            message_obj = self._client.messages.create(body=message, from_=self.phone_number, to=to_number)
            ...
            return NotificationResult(success=True, message_sid=message_obj.sid, status=message_obj.status)
    ...
    def send_whatsapp(
        self, to_number: str, message: str, content_variables: dict = None, force_resend: bool = False
    ) -> NotificationResult:
        """
        ...
        if not self.whatsapp_number:
            return NotificationResult(success=False, error="Numéro Twilio WhatsApp non configuré")
        ...
        try:
            ...
            if use_template and content_sid and content_variables:
                ...
                message_obj = self._client.messages.create(body=message, from_=whatsapp_from, to=whatsapp_to)
...
Motif: {fiche.motif_consultation or 'Non spécifié'}"""
        ...
        if fiche.assigned_medecin:
            medecin_name = (
                f"Dr. {fiche.assigned_medecin.get_full_name()}"
                if hasattr(fiche.assigned_medecin, "get_full_name")
                else f"Dr. {fiche.assigned_medecin.username}"
            )
...
🕐 {local_time.strftime('%H:%M:%S.%f')[:-3]}"""
    ...
    def clear_notification_cache(self, recipient: str = None, notification_type: str = None):
        ...
        if recipient:
            ...
            patterns = [f"whatsapp_sent_{recipient}_*", f"notification_*_{recipient}_*"]
...
def send_consultation_notification(fiche, method: str = "sms", force_resend: bool = False) -> NotificationResult:
    ...
    if not fiche.telephone:
        return NotificationResult(success=False, error="Numéro de téléphone non renseigné")
