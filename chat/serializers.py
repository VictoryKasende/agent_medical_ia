...
class AppointmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'consultation_mode', 'consultation_mode_display', 'location_note',
            'status', 'status_display', 'message_patient', 'message_medecin',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'status_display', 'consultation_mode_display', 'created_at', 'updated_at', 'confirmed_start', 'confirmed_end']
...
class FicheReferenceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'fiche', 'title', 'url', 'source', 'source_display',
            'authors', 'year', 'journal', 'created_at'
        ]
...
class LabResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LabResult
        fields = [
            'id', 'fiche', 'type_analyse', 'valeur', 'unite', 'valeurs_normales',
            'date_prelevement', 'laboratoire', 'fichier', 'file_url',
...
class FicheAttachmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'fiche', 'file', 'file_url', 'file_size', 'file_extension',
            'kind', 'kind_display', 'note', 'uploaded_by', 'uploaded_by_username',
            'created_at'
        ]
        read_only_fields = [
            'id', 'file_url', 'file_size', 'file_extension', 
            'kind_display', 'uploaded_by', 'uploaded_by_username', 'created_at'
        ]
...
class MedecinAvailabilitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'medecin', 'medecin_name', 'day_of_week', 'day_of_week_display',
            'start_time', 'end_time', 'consultation_type', 'consultation_type_display',
            'duration_minutes', 'duration_formatted', 'is_active', 'max_consultations',
    ...
    def validate(self, attrs):
        ...
        if attrs['duration_minutes'] <= 0:
            raise serializers.ValidationError({
                'duration_minutes': 'La durée doit être positive'
            })
...
class MedecinExceptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'medecin', 'medecin_name', 'start_datetime', 'end_datetime',
            'exception_type', 'exception_type_display', 'reason', 'is_recurring',
            'created_at'
        ]
...
class WebhookEventSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = WebhookEvent
        fields = [
            'id', 'event_type', 'event_type_display', 'external_id',
            'sender_phone', 'recipient_phone', 'content', 'raw_payload',
            'processing_status', 'processing_status_display', 'processing_error',
            'related_user', 'related_user_name', 'related_fiche', 'related_fiche_number',
            'created_message', 'received_at', 'processed_at'
        ]
        read_only_fields = [
            'id', 'received_at', 'processed_at', 'related_user_name', 
            'related_fiche_number', 'processing_status_display', 'event_type_display'
        ]
...
class DataExportJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DataExportJob
        fields = [
            'id', 'created_by', 'created_by_name', 'export_format', 'export_format_display',
            'date_start', 'date_end', 'include_personal_data', 'filters',
            'status', 'status_display', 'file_path', 'file_size', 'file_size_formatted',
    ...
    def validate(self, attrs):
        ...
        if (attrs['date_end'] - attrs['date_start']) > timedelta(days=730):
            raise serializers.ValidationError({
                'date_end': 'La plage d\'export ne peut pas dépasser 2 ans'
            })
