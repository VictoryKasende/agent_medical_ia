...
urlpatterns = [
    ...
    # (legacy consultations-distance HTML & legacy API removed; use DRF endpoints under /api/v1/)
    ...
    path(
        "consultation/patient/<int:fiche_id>/modifier/",
        views.FicheConsultationUpdateView.as_view(),
        name="fiche_consultation_update",
    ),
    path(
        "consultation/patient/<int:fiche_id>/modifier/statut",
        views.UpdateFicheStatusView.as_view(),
        name="fiche_consultation_update_status",
    ),
    path(
        "consultation/patient/<int:fiche_id>/details/",
        views.FicheConsultationDetailView.as_view(),
        name="fiche_consultation_detail",
    ),
    ...
    path(
        ...
        views.ConsultationPatientDistantView.as_view(),
        name="consultation_patient_distant",
    ),
    path(
        "consultation/patient-distant/<int:fiche_id>/modifier/",
        views.FicheConsultationUpdateView.as_view(),
        name="fiche_consultation_update_distant",
    ),
    # Envoyer un message WhatsApp
    path(
        "send-whatsapp/<int:consultation_id>/", views.send_whatsapp_message_view, name="send_whatsapp_message"
    ),  # ← CHANGÉ
]
